...
void loop() {
  ...
  if (now - lastReadMs >= READ_INTERVAL) {
    ...
    if (state == STABILIZING) {
      // Если температура вышла за допуск — сбрасываем отсчёт
      if (fabs(temperature - stabRefTemp) > STAB_TOLERANCE) {
        stabRefTemp = temperature;
        stabStartMs = now;
      }
      // Прошло 3 минуты без колебаний — фиксируем базу
      if (now - stabStartMs >= STAB_WINDOW_MS) {
        baseTemp       = roundTo1(temperature);
        alarmHighDelta = 0.0f;  // порог не задан, ждём первого нажатия
        state = WORKING;
        beepShort(2000, 150);
        delay(200);
        beepShort(2000, 150);
        Serial.printf("База зафиксирована: %.1f C\n", baseTemp);
      }
...
void updateAlarm() {
  if (alarmHighDelta < 0.001f) {
    // Порог ещё не задан кнопкой — тревоги нет
    alarmActive = false;
    return;
  }
...
void shortPress() {
  ...
  if (state == WORKING) {
    if (alarmHighDelta < 0.001f) {
      // Первое нажатие — устанавливаем начальный порог 0.2°C
      ...
      if (alarmHighDelta > ALARM_HIGH_MAX + 0.001f) {
        alarmHighDelta = ALARM_HIGH_START;  // по кругу возврат к 0.2
      }
    }
...
void drawWorking() {
  ...
  if (alarmHighDelta < 0.001f) {
    // Порог ещё не задан
    display.drawString(0, 41, "Hi:  -- нажмите кнопку");
  } else {
  ...
  if (alarmActive) {
    display.drawString(128, 41, "!!");
  } else if (alarmHighDelta < 0.001f) {
    display.drawString(128, 41, "--");
  } else {
    display.drawString(128, 41, "OK");
  }
